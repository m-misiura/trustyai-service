...
class PregeneratedDataLoader:
    ...
    def get_data(cls) -> np.ndarray:
        ...
        # if file exists in the test directory, use it
        if (test_dir / "data" / "pregenerated_normal_data.txt").exists():
            return cls.parse_java_array(str(test_dir / "data" / "pregenerated_normal_data.txt"))
    ...
    def get_t_stat_table(cls) -> np.ndarray:
        ...
        test_dir = Path(__file__).parent
        # if file exists in the test directory, use it
        if (test_dir / "data" / "t_stat_table.txt").exists():
            return cls.parse_java_array(str(test_dir / "data" / "t_stat_table.txt"))
    ...
    def get_p_value_table(cls) -> np.ndarray:
        ...
        test_dir = Path(__file__).parent
        # if file exists in the test directory, use it
        if (test_dir / "data" / "p_value_table.txt").exists():
            return cls.parse_java_array(str(test_dir / "data" / "p_value_table.txt"))
